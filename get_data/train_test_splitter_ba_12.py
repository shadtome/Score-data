...
class train_test:
    ...
    def get_splits(self,train_size,seed):
        ...
        final_date['12_months'] = final_date['final_date'] - pd.Timedelta(days = 365)
        
        temp = data.merge(final_date, on='name',how='left')
        temp_after = temp[temp['date']>=temp['12_months']]
        temp_before = temp[temp['date']<temp['12_months']]
    ...
    def save(self):
        ...
        test_fd = os.path.join(test_fd,'test_ba_12.csv')
        train_fd = os.path.join(train_fd,'train_ba_12.csv')
